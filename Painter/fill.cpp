...
void Fill::addFillPoint(QImage *image, int x, int y)
{
    ...
    if (notEdge(image, x + 1, y))
        addFillPoint(image, x + 1, y);
    if (notEdge(image, x - 1, y))
        addFillPoint(image, x - 1, y);
    if (notEdge(image, x, y + 1))
        addFillPoint(image, x, y + 1);
    if (notEdge(image, x, y - 1))
        addFillPoint(image, x, y - 1);
}
void Fill::paintShape(QPainter &p, QImage *image, bool isSave)
{
    ...
    if (points.size() < 2)
    {
        addFillPoint(image, points.first()->x(), points.first()->y());
    }
